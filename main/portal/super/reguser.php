...
if(isset($_POST['cemedt'])){
	...
	$id = $_POST['cemid'];
	$name = $_POST['cemidn'];
	$cem1 = mysqli_num_rows(mysqli_query($connection,"select * from transaction where user=$id"));
	if($cem1 > 0){
		echo "<script>alert('User $name can not be deleted due to its records in the transaction page. Please clear/update its record first.');</script>";
	}else{
...
<table id="datatable-responsive" class="table table-striped jambo_table table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
  ...
  <tbody>
  <?php
	$qnx = mysqli_query($connection,"select * from users where utype=2 and school=$sch");
