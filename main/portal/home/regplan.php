<?php
$xr1 = mysqli_fetch_array(mysqli_query($connection,"select sum(amount) as amt1 from transaction where ttype=1 and user=$uid and school=$sch and date>$rdate and approval!=0 and approval!=-1"));
$xr2 = mysqli_fetch_array(mysqli_query($connection,"select sum(amount) as amt2 from transaction where ttype=2 and user=$uid and school=$sch and date>$rdate and approval!=0 and approval!=-1"));
$xr3 = mysqli_fetch_array(mysqli_query($connection,"select sum(charges) as amt1 from transaction where ttype=1 and user=$uid and school=$sch and date>$rdate and approval!=0 and approval!=-1"));
$xr4 = mysqli_fetch_array(mysqli_query($connection,"select sum(charges) as amt2 from transaction where ttype=2 and user=$uid and school=$sch and date>$rdate and approval!=0 and approval!=-1"));
?>
...
<table id="datatable-responsive" class="table table-striped jambo_table table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
  ...
  <tbody>
  <?php
	$qnx = mysqli_query($connection,"select * from transaction where user=$uid and school=$sch order by date desc");
